...
plt.semilogy(np.ones((5))*3, np.linspace(lo_conf_cx, up_conf_cx,5), '-r', linewidth=3)
plt.semilogy(np.ones((5))*4, np.linspace(lo_conf_cy, up_conf_cy,5), '-r', linewidth=3)
plt.semilogy(np.ones((5))*5, np.linspace(lo_conf_sigma, up_conf_sigma,5), '-r', linewidth=3)
plt.semilogy(np.ones((5))*6, np.linspace(lo_conf_theta, up_conf_theta,5), '-r', linewidth=3)   
...
plt.semilogy(np.linspace(-0.1,0.1,5)+3, np.ones(5)*lo_conf_cx, '-r', linewidth=3)
plt.semilogy(np.linspace(-0.1,0.1,5)+3, np.ones(5)*up_conf_cx, '-r', linewidth=3)
plt.semilogy(np.linspace(-0.1,0.1,5)+4, np.ones(5)*lo_conf_cy, '-r', linewidth=3)
plt.semilogy(np.linspace(-0.1,0.1,5)+4, np.ones(5)*up_conf_cy, '-r', linewidth=3)
plt.semilogy(np.linspace(-0.1,0.1,5)+5, np.ones(5)*lo_conf_sigma, '-r', linewidth=3)
plt.semilogy(np.linspace(-0.1,0.1,5)+5, np.ones(5)*up_conf_sigma, '-r', linewidth=3)
plt.semilogy(np.linspace(-0.1,0.1,5)+6, np.ones(5)*lo_conf_theta, '-r', linewidth=3)
plt.semilogy(np.linspace(-0.1,0.1,5)+6, np.ones(5)*up_conf_theta, '-r', linewidth=3)
# true values and estimates
plt.semilogy(np.array([1,2,3,4,5,6]), np.array([r[0],r[1],c[0],c[1],sigma1,theta]), 'xk', label='true', markersize=10)
plt.scatter(np.array([1,2,3,4,5,6]), (np.array([r1_samples.mean(),r2_samples.mean(),cx_samples.mean(axis=0)\
                                            ,cy_samples.mean(axis=0),sigma_samples.mean(),theta_samples.mean()])), 60,facecolors='none', edgecolors='k',label='estimate')
# layout
plt.xticks(np.array([1,2,3,4,5,6]), ['$r_x$', '$r_y$', '$c_x$', '$c_y$', '$\kappa$', '$\delta$'], fontsize=20)
plt.gca().yaxis.set_major_formatter(FormatStrFormatter('%.2f cm'))
plt.yticks(fontsize=20)
plt.legend(fontsize=16)
plt.ylim([0.05,6])
plt.title('parameter UQ', fontsize=21)
plt.savefig('/zhome/ad/7/127239/Desktop/Kandidatspeciale/Problem6_rotated_ellipsoid_detection_new_bounds/Data/Figures/Problem62_2D_parameter_UQ.png',bbox_inches='tight')
