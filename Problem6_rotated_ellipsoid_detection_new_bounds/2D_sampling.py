...
class make_observation:
    def __init__(self, c, r, sigma_1, theta):
        ...
        self.m = 6                             # we choose M as a power of 2 to speed up fft computation.
        self.n = 2**self.m                          # Data mesh number of gridpoints
        self.mesh = UnitSquareMesh(self.n, self.n)       # Data mesh
        # Define spaces
            ...
            class sigmafun(UserExpression):
                def eval(self, values, x):
                    if (((x[0]-c[0])*np.cos(theta)+(x[1]-c[1])*np.sin(theta))**2/r[0]**2\
                        +((x[1]-c[1])*np.cos(theta)-(x[0]-c[0])*np.sin(theta))**2/r[1]**2)<=1:
                        values[0] = sigma_1
        ...
        def J_field(self):
            ...
            #(- self.sigma*grad(self.u)).vector().get_local()
            ...
            Jx = J_val[0:(self.n+1)**2].reshape(self.n+1, self.n+1)
            Jy = J_val[(self.n+1)**2:2*(self.n+1)**2].reshape(self.n+1, self.n+1)
        ...
        def gen_obs_no_crime(c,r,sigma1, theta):
            ...
            mesh = UnitSquareMesh(n, n)       # Data mesh
...
## make observation and do sampling            
np.random.seed(420)
r = np.array([0.15, 0.3])
...
within_loop_size = 20 
burn_in = 0
sampler = Metropolis_in_Gibbs_burn_in(obs.log_like, c0, r0, sigma1_0, theta0, np.array( [[0,1],[0,1]] ), np.array([0.05,0.5]), np.array([0,100]), np.array([0,np.pi]), beta, within_loop_size, burn_in)
...
if burn_in == 1:
	...
	print(betas)
	np.save('/zhome/ad/7/127239/Desktop/Kandidatspeciale/Problem6_rotated_ellipsoid_detection_new_bounds/Data/Samples/2D_samples_burnin_beta_values', betas)
else:
    np.save('/zhome/ad/7/127239/Desktop/Kandidatspeciale/Problem6_rotated_ellipsoid_detection_new_bounds/Data/Samples/final_2D_samples_c_beta_'+str(beta[0])+'_c0_'+str(c0[0])+'_'+str(c0[1]), samples_c)
    np.save('/zhome/ad/7/127239/Desktop/Kandidatspeciale/Problem6_rotated_ellipsoid_detection_new_bounds/Data/Samples/final_2D_samples_r_beta_'+str(beta[1])+'_r0_'+str(r0[0])+'_'+str(r0[1]), samples_r)
    np.save('/zhome/ad/7/127239/Desktop/Kandidatspeciale/Problem6_rotated_ellipsoid_detection_new_bounds/Data/Samples/final_2D_samples_sigma_beta_'+str(beta[2])+'_sigma0_'+str(sigma1_0[0]), samples_sigma1)
    np.save('/zhome/ad/7/127239/Desktop/Kandidatspeciale/Problem6_rotated_ellipsoid_detection_new_bounds/Data/Samples/final_2D_samples_theta_beta_'+str(beta[3])+'_theta0_'+str(theta0[0]), samples_theta)
