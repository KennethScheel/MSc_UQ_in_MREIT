...
class make_observation:
    def __init__(self, c, r):
        ...
        def solver(self):
            ...
            class sigmafun(UserExpression):
                def eval(self, values, x):
                    if (x[0] - c[0])**2 + (x[1] - c[1])**2 <= r**2 + DOLFIN_EPS:
                        values[0] = sigma_1
                    else:
                        values[0] = sigma_0    
        ...
        def gen_obs_no_crime(c,r):
            ...
            sigma_0 = Constant(1.0)   # conductivity of 1 outside ball
            sigma_1 = Constant(2.0)   # conductivity of 2 inside ball
            class sigmafun(UserExpression):
                def eval(self, values, x):
                    if (x[0] - c[0])**2 + (x[1] - c[1])**2 <= r**2 + DOLFIN_EPS:
                        values[0] = sigma_1
        ...
        self.y_obs, self.sigma_noise = gen_obs_no_crime(c,r)
        self.log_like = lambda c, r: -0.5*np.linalg.norm(self.y_obs - sigma_to_B(c,r))**2/self.sigma_noise**2
        def save_obs(self):
...
np.random.seed()
obs = make_observation(c,r)

r0, c0 = np.array([0.1]), np.array([0.8, 0.1])
beta = np.array([0.03, 0.06])
within_loop_size = 20 
burn_in = 0
sampler = Metropolis_in_Gibbs_burn_in(obs.log_like, c0, r0, np.array( [[0,1],[0,1]] ), np.array([0.05,0.5]), beta, within_loop_size, burn_in)
sampler.sample(5000, 20)
samples_c, samples_r = sampler.give_stats()

if burn_in == 1:
	betas = np.array([sampler.beta1, sampler.beta2])
	print(betas)
	np.save('/zhome/ad/7/127239/Desktop/Kandidatspeciale/Toy_problem_one_ball/Data/Samples/2D_samples_burnin_beta_values', betas)
else:
	np.save('/zhome/ad/7/127239/Desktop/Kandidatspeciale/Toy_problem_one_ball/Data/Samples/final_2D_samples_c_beta_'+str(beta[0])+'_c0_'+str(c0[0])+'_'+str(c0[1]), samples_c)
	np.save('/zhome/ad/7/127239/Desktop/Kandidatspeciale/Toy_problem_one_ball/Data/Samples/final_2D_samples_r_beta_'+str(beta[1])+'_r0_'+str(r0[0]), samples_r)
