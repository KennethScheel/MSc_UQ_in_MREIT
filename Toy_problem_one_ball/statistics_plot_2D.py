...
plt.plot(np.array([1,2,3]), np.array([r,c[0],c[1]]), 'xk', label='true', markersize=10)
plt.scatter(np.array([1,2,3]), (np.array([r1_samples.mean(),cx_samples.mean(axis=0)\
                                            ,cy_samples.mean(axis=0)])), 60,facecolors='none', edgecolors='k',label='estimate')
# layout
plt.xticks(np.array([1,2,3]), ['$r$', '$c_x$', '$c_y$'], fontsize=20)
plt.gca().yaxis.set_major_formatter(FormatStrFormatter('%.2f cm'))
plt.yticks(fontsize=16)
plt.legend(fontsize=16)
plt.ylim([0.1,0.6])
plt.title('parameter UQ', fontsize=21)
plt.savefig('/zhome/ad/7/127239/Desktop/Kandidatspeciale/Toy_problem_one_ball/Data/Figures/toy1_2D_parameter_UQ.png',bbox_inches='tight')
