...
class make_observation:
    def __init__(self, c, r):
        ...
        self.sigma_0 = Constant(1.0)   # conductivity of 1 outside ball
        self.sigma_1 = Constant(2.0)   # conductivity of 2 inside ball
        ...
        def solver(self):
            ...
            class sigmafun(UserExpression):
                def eval(self, values, x):
                    # one circle
                    if (x[0] - c[0])**2 + (x[1] - c[1])**2 + (x[2] - c[2])**2 <= r**2 + DOLFIN_EPS:
                        values[0] = sigma_1
                    else:
                        values[0] = sigma_0
        ...
        def gen_obs_no_crime(c,r):
            ...
            sigma_1 = 2.0    # conductivity of 2 inside ball
            class sigmafun(UserExpression):
                def eval(self, values, x):
                    # one circle
                    if (x[0] - c[0])**2 + (x[1] - c[1])**2 + (x[2] - c[2])**2 <= r**2 + DOLFIN_EPS:
...
within_loop_size = 20 
burn_in = 0
sampler = Metropolis_in_Gibbs_burn_in(obs.log_like, c0, r0, np.array( [[0,1],[0,1],[0,1]] ), np.array([0.05,0.5]), beta, within_loop_size, burn_in)
...
if burn_in == 1:
	...
	print(betas)
	np.save('/zhome/ad/7/127239/Desktop/Kandidatspeciale/Toy_problem_one_ball/Data/Samples/3D_samples_burnin_beta_values', betas)
else:
    np.save('/zhome/ad/7/127239/Desktop/Kandidatspeciale/Toy_problem_one_ball/Data/Samples/final_3D_samples_c_beta_'+str(beta[0])+'_c0_'+str(c0[0])+'_'+str(c0[1])+'_'+str(c0[2]), samples_c)
    np.save('/zhome/ad/7/127239/Desktop/Kandidatspeciale/Toy_problem_one_ball/Data/Samples/final_3D_samples_r_beta_'+str(beta[1])+'_r0_'+str(r0[0]), samples_r)
